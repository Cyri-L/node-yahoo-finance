...
function _sanitizeHistoricalOptions(options) {
  ...
  if (!options.from) {
    options.from = moment.utc('1900-01-01');
  }
...
function splitsAndDivs(options, cb) {
  ...
  return Promise.resolve()
    ...
    .map(function(symbol) {
      return _utils.download(_constants.HISTORICAL_SPITS_AND_DIVIDENDS, {
          s: encodeURIComponent(symbol),
          a: options.from.format('MM') - 1,
          b: options.from.format('DD'),
          c: options.from.format('YYYY'),
        ...
        .then(_utils.parseCSV)
        .then(function(data) {
          return _transformSplitsAndDivs(symbol, data);
        })
        .catch(function(err) {
          if (options.error) {
            throw err;
...
function _transformSplitsAndDivs(symbol, data) {
  var headings = data.shift();
  var parts, ret;
  headings.unshift("Type");
  headings[2] = "Value"; //instead of Dividends
  ret = _(data)
    .reverse()
    .filter(function(item) {
      return item[0] == "DIVIDEND" || item[0] == "SPLIT"; //check type
    });
